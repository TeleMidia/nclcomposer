...
interativas. Para isso, ele utiliza-se de várias abstrações gráficas
(visões) que permitem ao autor focar em apenas um determinado ponto, durante
cada fase distinta da autoria.
...
Para instalar o {nclcomposer} em ambiente Windows:
  ...
  . Baixe a versão mais atual do {nclcomposer} para Windows em:
	...
	X.X.X deve conter a versão do {nclcomposer}. Execute esse arquivo, e a
	seguinte tela deve ser apresentada:
	TODO: Image.
	. Siga o passo a passo do instalador até terminar a instalação.
	. Caso tenha instalado corretamente é possível executar o {nclcomposer}
	através do menu _Iniciar->NCL Composer->NCL Composer_.

...
e também as bases de conectores. Por enquanto, apenas deixe esta opção marcada,
pois ela será útil para os próximos exemplos.
...
Com o primeiro projeto criado, pode-se ter acesso às visões que já vem instaladas
por _default_ no {nclcomposer}: {default_views}.
...
para a visão _Textual_, por exemplo, poderá ver o código XML do documento: 
...
Para finalizar este primeiro exemplo só falta agora definirmos
onde o vídeo da animação será exibido. A forma mais simples de

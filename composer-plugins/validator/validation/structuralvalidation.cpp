 ...
 **********************************************************************************/
void testAttributes (const ModelElement &el, Model &model, vector<pair<void *, string> > &msgs, Message &messageFactory) {
    ...
    for (Attribute att; attsIt != declaredAtts.end (); declaredAttsSet.insert (att.name()), ++attsIt) {
        ...
        if (atts.count (att.name()) == 0) {
            ...
            msgs.push_back(pair<void *, string> (el.data(), messageFactory.createMessage(2002, 2, att.name().c_str(),
        ...
        if (!Langstruct::isValidAttribute(att.name(), att.value(), el.elementName())){
            ...
            msgs.push_back(pair<void *, string> (el.data(), messageFactory.createMessage(2004, 2, att.name().c_str(),
                                                                                        el.elementName().c_str())));
            model.addElementWithErrorInLastPass(el.id());
        }
        ...
        if (declaredAttsSet.count (att.name ()) != 0){
            fprintf (stderr, "'%s' attribute is already declared in element '%s'\n",
                     att.value().c_str (), el.elementName().c_str ());
            msgs.push_back(pair<void *, string> (el.data(), messageFactory.createMessage(2013, 2, att.name().c_str(),
                                                                                        el.elementName().c_str())));
            model.addElementWithErrorInLastPass(el.id());
        }
    }
    ...
    for (; it != atts.end (); ++it) {
        ...
        if ((*it).second && declaredAttsSet.count(att) == 0){
            ...
            msgs.push_back(pair<void *, string> (el.data(),
                                                messageFactory.createMessage(2003, 2, att.c_str(), el.elementName().c_str())));
            model.addElementWithErrorInLastPass(el.id());
        }
    }
...
void testCardinality (const ModelElement &el, Model &model, map<string, int> &cardinalityMap,
    ...
    for (char cardinality = ' '; childNameIt != childrenNames.end (); ++childNameIt) {
        ...
        case '1': if (cardinalityMap[*childNameIt] != 1){
                ...
                msgs.push_back(pair<void *, string> (el.data(), messageFactory.createMessage(2008, 2, el.elementName().c_str(),
                                                                                            childNameIt->c_str())));
                model.addElementWithErrorInLastPass(el.id());
                }
            break;
        ...
        case '?': if (cardinalityMap[*childNameIt] > 1){
                ...
                msgs.push_back(pair<void *, string> (el.data(),
                                                    messageFactory.createMessage(2006, 2, el.elementName().c_str(),
                                                                                 childNameIt->c_str())));
                model.addElementWithErrorInLastPass(el.id());
                }
            break;
        ...
        case '+': if (cardinalityMap[*childNameIt] < 1){
                fprintf (stderr, "'%s', child of '%s', MUST be declared at least one time\n",
                         (*childNameIt).c_str (), el.elementName().c_str ());
                msgs.push_back(pair<void *, string> (el.data(),
                                                    messageFactory.createMessage(2007, 2, el.elementName().c_str(),
                                                                                 childNameIt->c_str())));
                model.addElementWithErrorInLastPass(el.id());
                }
            break;
 ...
 **********************************************************************************/
void StructuralValidation::structuralValidation(const ModelElement &el, Model &model, vector<pair<void *, string> > &msgs, Message &messageFactory)
{
    ...
    if (!Langstruct::existElement(el.elementName())) {
        ...
        msgs.push_back(pair<void *, string> (el.data(), messageFactory.createMessage(2001, 1, el.elementName().c_str())));
        model.addElementWithErrorInLastPass(el.id());
    ...
    if (!Langstruct::existParent(el.elementName(), parent)) {
        ...
        msgs.push_back(pair<void *, string> (el.data(),
                                            messageFactory.createMessage(2005, 2, el.elementName().c_str(), parent.c_str())));
        model.addElementWithErrorInLastPass(el.id());
    }

...
public:
  ...
  inline const vector <virtualId> references () const
  {
...
public:
  ...
  inline void addElementWithErrorInLastPass (virtualId id)
  {
    _elementsWithErrorInLastPass.insert(id);
  }
  inline void clearElementsWithErrorInLastPass ()
  {
  ...
  inline bool hasElement (virtualId &id)
  {
    return _modelElements.count (id) > 0;
  }
  inline void clearMarkedElements ()
  {
    _markedElements.clear(); _affectedEllements.clear();
  }
  inline const set <virtualId> & markedElements () { return _markedElements; }

  inline void setRelativePath (const string & relativePath)
  {

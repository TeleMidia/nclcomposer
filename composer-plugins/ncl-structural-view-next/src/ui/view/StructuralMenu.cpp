...
void StructuralMenu::createActions()
{
  ...
  actionAddMedia->setData(Structural::Media);
  actionAddMedia->setIcon(QIcon(StructuralUtil::iconFromMediaType(Structural::Media)));
  actionAddMedia->setEnabled(false);

  // add context action
  actionAddContext = new QAction(this);
  actionAddContext->setText(tr("Context"));
  actionAddContext->setData(Structural::Context);
  actionAddContext->setIcon(QIcon(StructuralUtil::iconFromMediaType(Structural::Context)));
  actionAddContext->setEnabled(false);

  // add switch action
  actionAddSwitch = new QAction(this);
  actionAddSwitch->setText(tr("Switch"));
  actionAddSwitch->setData(Structural::Switch);
  actionAddSwitch->setIcon(QIcon(StructuralUtil::iconFromMediaType(Structural::Switch)));
  actionAddSwitch->setEnabled(false);

  // add body action
  actionAddBody = new QAction(this);
  actionAddBody->setText(tr("Body"));
  actionAddBody->setData(Structural::Body);
  actionAddBody->setIcon(QIcon(StructuralUtil::iconFromMediaType(Structural::Body)));
  actionAddBody->setEnabled(false);

  // add port action
  actionAddPort = new QAction(this);
  actionAddPort->setText(tr("Port"));
  actionAddPort->setData(Structural::Port);
  actionAddPort->setIcon(QIcon(StructuralUtil::iconFromMediaType(Structural::Port)));
  actionAddPort->setEnabled(false);

  // add mapping action
  actionAddSwitchPort = new QAction(this);
  actionAddSwitchPort->setText(tr("Switch Port"));
  actionAddSwitchPort->setData(Structural::SwitchPort);
  actionAddSwitchPort->setIcon(QIcon(StructuralUtil::iconFromMediaType(Structural::SwitchPort)));
  actionAddSwitchPort->setEnabled(false);

  // area action
  actionAddArea = new QAction(this);
  actionAddArea->setText(tr("Area"));
  actionAddArea->setData(Structural::Area);
  actionAddArea->setIcon(QIcon(StructuralUtil::iconFromMediaType(Structural::Area)));
  actionAddArea->setEnabled(false);

  // property action
  actionAddProperty = new QAction(this);
  actionAddProperty->setText(tr("Property"));
  actionAddProperty->setData(Structural::Property);
  actionAddProperty->setIcon(QIcon(StructuralUtil::iconFromMediaType(Structural::Property)));
  actionAddProperty->setEnabled(false);

  // aggregator action
  actionAddAggregator = new QAction(this);
  actionAddAggregator->setText(tr("Link"));
  actionAddAggregator->setData(Structural::Link);
  actionAddAggregator->setIcon(QIcon(StructuralUtil::iconFromMediaType(Structural::Link)));
...
void StructuralMenu::dispatchAddAction()
{
  QAction *action = dynamic_cast <QAction*> (QObject::sender());
  if(action != NULL)
  {
    emit menuAddEntityTriggered( (Structural::EntitySubtype) action->data().toInt() );
  }
}

...
void
tst_ModulePlugin::cleanupTestCase ()
{
...
void
tst_ModulePlugin::loadPluginDir ()
{
...
void
tst_ModulePlugin::pluginName_data ()
{
...
#elif WIN32
  QTest::newRow ("NCL") << profileDir + QDir::separator () + "NCL"
                               + QDir::separator () + "NCLLanguageProfile.dll";
#else
...
void
tst_ModulePlugin::pluginLoadBenchmark_data ()
{
  QTest::addColumn<QString> ("fileName");
...
void
tst_ModulePlugin::pluginLoadBenchmark ()
{
  if (!isBenchmark)
  ...
  QBENCHMARK { pgControl->loadPlugin (fileName); }
}

...
PerspectiveManager::PerspectiveManager (QWidget *parent)
    : QDialog (parent), ui (new Ui::PerpectiveManager ())
{
  ui->setupUi (this);
  ...
  connect (ui->deleteButton, SIGNAL (clicked ()), this,
           SLOT (deleteSelectedPerspective ()));

  connect (ui->treeWidget, SIGNAL (itemChanged (QTreeWidgetItem *, int)), this,
           SLOT (itemChanged (QTreeWidgetItem *, int)));

  GlobalSettings settings;
  defaultPerspective = settings.value ("default_perspective").toString ();
}

PerspectiveManager::~PerspectiveManager () { delete ui; }

void
PerspectiveManager::setBehavior (PERSPEC_BEHAVIOR behavior)
{
  this->behavior = behavior;
}

void
PerspectiveManager::updateContent ()
{
  GlobalSettings settings;
...
void
PerspectiveManager::showEvent (QShowEvent *evt)
{
...
void
PerspectiveManager::itemChanged (QTreeWidgetItem *item, int col)
{
  if (internalChange)
    return;
...
QString
PerspectiveManager::getSelectedName ()
{
  return selectedName;
}

QString
PerspectiveManager::getDefaultPerspective ()
{
  return defaultPerspective;
}

void
PerspectiveManager::deletePerspective (QString name)
{
  GlobalSettings settings;
  settings.beginGroup ("pluginslayout");
...
void
PerspectiveManager::deleteSelectedPerspective ()
{
  QList<QTreeWidgetItem *> items = ui->treeWidget->selectedItems ();
  selectedName = items.at (0)->text (1);

...
void
PreferencesEditor::loadPreferences ()
{
  int i = 0;
  ...
  while (!categories.isEmpty ())
  {
    ...
    QList<Preference *> preferences
        = Preferences::instance ()->preferences (category);
    ...
    while (!preferences.isEmpty ())
    {
      ...
      item->setFlags (item->flags ()
                      ^ Qt::ItemIsEditable); // First column non-editable

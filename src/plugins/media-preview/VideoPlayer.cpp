...
VideoPlayer::VideoPlayer(const QString &filename): ui(new Ui::VideoPlayer)
{
  setMinimumWidth(430);
  ...
  ui->volumeSlider->setSizePolicy (QSizePolicy::Maximum,
  ...
  connect(ui->videoDisplay, SIGNAL(finished()),
          ui->videoDisplay, SLOT(deleteLater()));
  connect(ui->playbutton, SIGNAL(clicked()),
          ui->videoDisplay, SLOT(on_playbutton_clicked()));
  connect(ui->pausebutton, SIGNAL(clicked()),
          ui->videoDisplay, SLOT(on_paused_clicked()));
  connect(ui->stopbutton, SIGNAL(clicked()),
          ui->videoDisplay, SLOT(on_stop_clicked()));
}

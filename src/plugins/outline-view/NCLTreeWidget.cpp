...
NCLTreeWidget::NCLTreeWidget (QWidget *parent) : QTreeWidget (parent)
{
  ...
  // Extension Icons
  _extIcons = {
    { "txt", ":/icon/media-text" },       { "htm", ":/icon/media-text-html" },
    { "html", ":/icon/media-text-html" }, { "gif", ":/icon/media-image" },
    { "png", ":/icon/media-image" },      { "jpg", ":/icon/media-image" },
    { "jpeg", ":/icon/media-image" },     { "gif", ":/icon/media-image" },
    { "mp3", ":/icon/media-audio" },      { "wav", ":/icon/media-audio" },
    { "ac3", ":/icon/media-audio" },      { "mpa", ":/icon/media-audio" },
    { "mp2", ":/icon/media-audio" },      { "mp4", ":/icon/media-video" },
    { "avi", ":/icon/media-video" },      { "mpeg4", ":/icon/media-video" },
    { "mpeg", ":/icon/media-video" },     { "mpg", ":/icon/media-video" },
    { "mov", ":/icon/media-video" },      { "ts", ":/icon/media-video" },
    { "ncl", ":/icon/media-ncl" },        { "lua", ":/icon/media-nclua" }
  };
...
void
NCLTreeWidget::createActions ()
{
  _insertNodeAct
      = new QAction (QIcon (":/icon/plus"), tr ("&Add child"), this);
  connect (_insertNodeAct, SIGNAL (triggered ()), this,
           SLOT (userAddNewElement ()));
  addAction (_insertNodeAct);

  _removeNodeAct = new QAction (QIcon (":/icon/minus"),
                                tr ("&Remove selected element"), this);
  ...
  connect (_removeNodeAct, SIGNAL (triggered ()), this,
           SLOT (userRemoveElement ()));
  ...
  _openWithDefaultEditorAct
      = new QAction (tr ("&Open with system default editor"), this);
  _openWithDefaultEditorAct->setEnabled (false);

  connect (_openWithDefaultEditorAct, SIGNAL (triggered ()),
           SLOT (openWithDefaultSystemEditor ()));
  addAction (_openWithDefaultEditorAct);
}

void
NCLTreeWidget::createMenus ()
{
...
bool
NCLTreeWidget::updateFromText (QString text)
{
  ...
  connect (handler, SIGNAL (fatalErrorFound (QString, QString, int, int, int)),
           this, SLOT (errorNotification (QString, QString, int, int, int)));
...
QTreeWidgetItem *
NCLTreeWidget::addElement (QTreeWidgetItem *father, int pos, QString tagname,
                           QString id, QMap<QString, QString> &attrs,
                           int /* line_in_text */, int /* column_in_text*/)
{
...
void
NCLTreeWidget::userAddNewElement ()
{
  bool ok;
...
QTreeWidgetItem *
NCLTreeWidget::getItemById (QString itemId)
{
  QList<QTreeWidgetItem *> items
      = findItems (itemId, Qt::MatchExactly | Qt::MatchRecursive, 2);
...
void
NCLTreeWidget::userRemoveElement ()
{
  QList<QTreeWidgetItem *> selecteds = this->selectedItems ();
...
void
NCLTreeWidget::updateItem (QTreeWidgetItem *item, QString tagname,
...
void
NCLTreeWidget::errorNotification (QString message, QString filename, int line,
...
void
NCLTreeWidget::resetFont ()
{
  setFont (_defaultFont);
}

void
NCLTreeWidget::setDefaultFont (const QFont &defaultFont)
{
  this->_defaultFont = defaultFont;
}

void
NCLTreeWidget::wheelEvent (QWheelEvent *event)
{
...
void
NCLTreeWidget::keyPressEvent (QKeyEvent *event)
{
  if (event->modifiers () & Qt::ControlModifier)
  {
...
void
NCLTreeWidget::resetZoom ()
{
  resetFont ();
}

void
NCLTreeWidget::mouseMoveEvent (QMouseEvent *event)
{
  ...
  if (selectedItem)
  {
    ...
    mimeData->setData ("nclcomposer/mediaid",
                       currentItem ()->text (3).toLatin1 ());
    mimeData->setData ("nclcomposer/qnstuid",
                       currentItem ()->text (2).toLatin1 ());
...
void
NCLTreeWidget::handleSelectionChanged ()
{
  QList<QTreeWidgetItem *> selecteds = this->selectedItems ();

  if (selecteds.size () && (selecteds.at (0)->text (4) == "media"
...
void
NCLTreeWidget::openWithDefaultSystemEditor ()
{
  QList<QTreeWidgetItem *> selecteds = this->selectedItems ();

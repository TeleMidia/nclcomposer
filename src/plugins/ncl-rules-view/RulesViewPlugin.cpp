...
void
RulesViewPlugin::onEntityRemoved (const QString &pluginID,
...
void
RulesViewPlugin::releaseItemChildren (QTreeWidgetItem *item)
{
  _items.remove (item);
...
void
RulesViewPlugin::findAllRules (Entity *root)
{
  if (!root)
    return;
...
void
RulesViewPlugin::addRule (Entity *entity)
{
  if (!entity)
    return;
  ...
  if (parent)
  {
    ...
    {
      item = new RuleItem (parent, entity->getAttribute (ID_ATTR),
                           entity->getAttribute (VAR_ATTR),
                           entity->getAttribute (COMPARATOR_ATTR),
                           entity->getAttribute (VALUE_ATTR), RULE_TYPE,
                           _rulesTable);
    ...
    {
      item = new CompositeRuleItem (parent, entity->getAttribute (ID_ATTR),
...
void
RulesViewPlugin::updateValue (QTreeWidgetItem *item)
{
  if (!item)
    return;
  ...
  switch (item->type ())
  {
    ...
    case RULE_TYPE:
    {
      ...
      if (newId != oldId || newVar != oldVar || newComparator != oldComparator
          || newValue != oldValue)
      {
...
void
RulesViewPlugin::sendSelectionChangedSignal ()
{
...
void
RulesViewPlugin::sendRemoveEntitySignal (QTreeWidgetItem *item)
{
  if (!item)
    return;
...
void
RulesViewPlugin::sendAddEntitySignal (QTreeWidgetItem *parent, int type)
{
  QMap<QString, QString> attr;
  QString parentUId;
  ...
  if (!parent)
  {
    if (type == RULEBASE_TYPE)
    {
      ...
      {
        QMessageBox::information (_rulesTable, "Ops...",

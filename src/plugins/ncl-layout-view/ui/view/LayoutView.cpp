...
LayoutView::LayoutView (QWidget *parent)
    : QStackedWidget (parent), gridVisibility (false)
{
...
void
LayoutView::requestRegionAddition (const QString regionUID,
...
void
LayoutView::requestRegionChange (const QString regionUID,
                                 const QString regionbaseUID,
                                 QMap<QString, QString> attributes)
{

  emit regionChanged (regionUID, regionbaseUID, attributes);
}

void
LayoutView::performSwitch (QAction *action)
{
  emit regionBaseSelected (_regionbaseActions.key (action));
}

void
LayoutView::performRegionBase ()
{
  QMap<QString, QString> attributes;
...
void
LayoutView::addRegion (const QString regionUID, const QString parentUID,
                       const QString regionbaseUID,
...
void
LayoutView::performCopy (LayoutRegion *region)
{
  // setting
...
void
LayoutView::performCut (LayoutRegion *region)
{
  performCopy (region);
  if (region != NULL)
  {
...
LayoutView::performPaste ()
{
  ...
  if (_selectedRegion != NULL)
  {
    ...
    if (_selectedRegion->isCloning ()) // being called by
                                       // ctrl+mousepressmove,which clones
                                       // region
    {
      // selectedRegion->getCloneAttributes();
      copiedRegionAttrs["top"] = _selectedRegion->getCloneAttributes ()["top"];
      copiedRegionAttrs["left"]
          = _selectedRegion->getCloneAttributes ()["left"];
      copiedRegionAttrs["right"]
          = _selectedRegion->getCloneAttributes ()["right"];
      copiedRegionAttrs["bottom"]
          = _selectedRegion->getCloneAttributes ()["bottom"];
      copiedRegionAttrs["zIndex"]
          = _selectedRegion->getCloneAttributes ()["zIndex"];
  ...
  requestRegionAddition ("", selectedRegionUId, selectedRegionBaseUId,
                         copiedRegionAttrs);
}

void
LayoutView::removeRegion (const QString regionUID, const QString regionbaseUID)
{
  if (_regions.contains (regionUID) && _regionbases.contains (regionbaseUID))
  {
    removeRegion (_regions[regionUID], _regionbases[regionbaseUID]);
  }
}

void
LayoutView::changeRegion (const QString regionUID, const QString regionbaseUID,
                          const QMap<QString, QString> attributes)
{
  if (_regions.contains (regionUID) && _regionbases.contains (regionbaseUID))
  {
    changeRegion (_regions[regionUID], _regionbases[regionbaseUID],
                  attributes);
  }
}

void
LayoutView::selectRegion (const QString regionUID, const QString regionbaseUID)
{
  if (_regions.contains (regionUID) && _regionbases.contains (regionbaseUID))
  {
    selectRegion (_regions[regionUID], _regionbases[regionbaseUID]);
  }
}

void
LayoutView::addDescriptor (const QString regionUID, const QString descriptor)
{
...
void
LayoutView::addRegionBase (const QString regionbaseUID,
                           const QMap<QString, QString> attributes)
{
  if (!_regionbases.contains (regionbaseUID))
  {
    ...
    LayoutRegionBase *regionbase = new LayoutRegionBase (canvas, _switchMenu);
...
void
LayoutView::removeRegionBase (const QString regionbaseUID)
{
  if (_regionbases.contains (regionbaseUID))
  {
...
void
LayoutView::changeRegionBase (const QString regionbaseUID,
                              const QMap<QString, QString> attributes)
{
  if (_regionbases.contains (regionbaseUID))
  {
...
void
LayoutView::selectRegionBase (const QString regionbaseUID)
{
  if (_regionbases.contains (regionbaseUID))
  {
    ...
    qCWarning (CPR_PLUGIN_LAYOUT)
        << tr ("It is not possible to select regionBase ") << regionbaseUID;
}

void
LayoutView::addRegion (LayoutRegion *region, LayoutRegion *parent,
                       ...
                       const QMap<QString, QString> attributes)
{
  if (region != NULL && regionBase != NULL)
  {
    ...
    regionBase->addRegion (region, parent, attributes);
  }
}

void
LayoutView::removeRegion (LayoutRegion *region, LayoutRegionBase *regionBase)
{
  if (region != NULL && regionBase != NULL)
  {
...
void
LayoutView::changeRegion (LayoutRegion *region, LayoutRegionBase *regionBase,
                          const QMap<QString, QString> attributes)
{
  if (region != NULL && regionBase != NULL)
  {
...
void
LayoutView::selectRegion (LayoutRegion *region, LayoutRegionBase *regionBase)
{
...
void
LayoutView::addRegionBase (LayoutRegionBase *regionBase,
                           const QMap<QString, QString> attributes)
{
  if (regionBase != NULL)
  {
    ...
    connect (
        regionBase, SIGNAL (regionChangeRequested (QString, QString,
                                                   QMap<QString, QString>)),
        SLOT (requestRegionChange (QString, QString, QMap<QString, QString>)));

    connect (regionBase, SIGNAL (regionSelectionRequested (QString, QString)),
             SIGNAL (regionSelected (QString, QString)));

    connect (regionBase, SIGNAL (regionBaseSelectionRequested (QString)),
             SIGNAL (regionBaseSelected (QString)));

    connect (regionBase, SIGNAL (regionBaseDeletionRequested (QString)),
             SIGNAL (regionBaseRemoved (QString)));

    connect (regionBase, SIGNAL (regionDeletionRequested (QString, QString)),
             SIGNAL (regionRemoved (QString, QString)));

    connect (regionBase, SIGNAL (mediaOverRegion (QString, QString)),
             SIGNAL (mediaOverRegionAction (QString, QString)));

    regionBase->setGridVisible (gridVisibility);
    connect (regionBase, SIGNAL (gridVisibilityChanged (bool)),
             SLOT (setGridVisible (bool)));

    connect (regionBase, SIGNAL (copyRequested (LayoutRegion *)),
             SLOT (performCopy (LayoutRegion *)));

    connect (regionBase, SIGNAL (cutRequested (LayoutRegion *)),
             SLOT (performCut (LayoutRegion *)));

    connect (regionBase, SIGNAL (pasteRequested ()), SLOT (performPaste ()));
...
void
LayoutView::removeRegionBase (LayoutRegionBase *regionBase)
{
  if (regionBase != NULL)
  {
...
void
LayoutView::changeRegionBase (LayoutRegionBase *regionBase,
                              const QMap<QString, QString> attributes)
{
  if (regionBase != NULL)
  {
...
void
LayoutView::selectRegionBase (LayoutRegionBase *regionBase)
{
  if (regionBase != NULL)
  {
    QWidget *parent = (QWidget *)regionBase->parent ();
...
void
LayoutView::setGridVisible (bool visible)
{
...
LayoutRegionBase *
LayoutView::getSelectedRegionBase ()
{
  return _selectedRegionBase;
}

LayoutRegion *
LayoutView::getSelectedRegion ()
{
  return _selectedRegion;
}

void
LayoutView::contextMenuEvent (QContextMenuEvent *event)
{

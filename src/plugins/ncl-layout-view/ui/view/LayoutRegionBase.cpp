...
QString
LayoutRegionBase::getId () const
{
  return _id;
}

void
LayoutRegionBase::setId (const QString &id)
{
  this->_id = id;
...
QString
LayoutRegionBase::getUid () const
{
  return _uid;
}

void
LayoutRegionBase::setUid (const QString &uid)
{
  this->_uid = uid;
}

QString
LayoutRegionBase::getRegion () const
{
  return _region;
}

void
LayoutRegionBase::setRegion (const QString &region)
{
  this->_region = region;
}

QString
LayoutRegionBase::getDevice () const
{
  return _device;
}

void
LayoutRegionBase::setDevice (const QString &device)
{
  this->_device = device;
}

void
LayoutRegionBase::selectRegion (LayoutRegion *region)
{
...
void
LayoutRegionBase::changeRegion (LayoutRegion *region,
...
void
LayoutRegionBase::requestRegionChange (LayoutRegion *region,
...
void
LayoutRegionBase::requestRegionSelection (LayoutRegion *region)
{
  emit regionSelectionRequested (region->getUid (), _uid);
}

void
LayoutRegionBase::LayoutRegionBase::createActions ()
{
  // help action
  ...
  while (!reslist.isEmpty ())
  {
    ...
    res->setData (
        QSize (QVariant (wh[0]).toInt (), QVariant (wh[1]).toInt ()));
...
void
LayoutRegionBase::createConnections ()
{
...
void
LayoutRegionBase::performShow (QAction *action)
{
  if (!action->isChecked ())
...
void
LayoutRegionBase::requestAdditionRegion (LayoutRegion *parent)
{
  QMap<QString, QString> attributes;
...
void
LayoutRegionBase::requestRegionDeletion (LayoutRegion *region)
{
  emit regionDeletionRequested (region->getUid (), _uid);
}

void
LayoutRegionBase::updateActionText (LayoutRegion *region)
{
  // Update Show Menu
...
void
LayoutRegionBase::hideRegion (LayoutRegion *region)
{
  _regionActions[region->getUid ()]->trigger ();
}

void
LayoutRegionBase::removeRegion (LayoutRegion *region)
{
  if (region != NULL)
  {
    if (region->parentItem () != NULL)
    {
      LayoutRegion *parent = (LayoutRegion *)region->parentItem ();
...
QGraphicsItem *
LayoutRegionBase::getBackgroundItem ()
{
  return _bgrect;
}

void
LayoutRegionBase::performDelete ()
{
  emit regionBaseDeletionRequested (_uid);
}

void
LayoutRegionBase::performRegion ()
{
  QMap<QString, QString> attributes
      = { { "top", "10%" },    { "left", "10%" },  { "right", "10%" },
          { "bottom", "10%" }, { "width", "80%" }, { "height", "80%" } };
...
void
LayoutRegionBase::addRegion (LayoutRegion *region, LayoutRegion *parent,
                             const QMap<QString, QString> attributes)
{
  if (region != NULL)
  {
    ...
    connect (region, SIGNAL (regionSelectionRequested (LayoutRegion *)),
             SLOT (requestRegionSelection (LayoutRegion *)));

    connect (
        region, SIGNAL (regionChangeRequested (LayoutRegion *,
                                               QMap<QString, QString>)),
        SLOT (requestRegionChange (LayoutRegion *, QMap<QString, QString>)));

    connect (region, SIGNAL (regionbasePerformed ()),
             SIGNAL (regionbasePerformed ()));

    connect (region, SIGNAL (regionAdditionRequested (LayoutRegion *)),
             SLOT (requestAdditionRegion (LayoutRegion *)));

    connect (region, SIGNAL (regionDeletionRequested (LayoutRegion *)),
             SLOT (requestRegionDeletion (LayoutRegion *)));

    connect (region, SIGNAL (dragMediaOverRegion (QString, LayoutRegion *)),
             this,
             SLOT (requestMediaOverRegionAction (QString, LayoutRegion *)));

    connect (region, SIGNAL (copyRequested (LayoutRegion *)),
             SIGNAL (copyRequested (LayoutRegion *)));

    connect (region, SIGNAL (cutRequested (LayoutRegion *)),
             SIGNAL (cutRequested (LayoutRegion *)));

    connect (region, SIGNAL (pasteRequested ()), SIGNAL (pasteRequested ()));
...
void
LayoutRegionBase::performChangeResolution ()
{
...
void
LayoutRegionBase::changeResolution (int w, int h)
{
...
void
LayoutRegionBase::performExport ()
{
...
void
LayoutRegionBase::mousePressEvent (QGraphicsSceneMouseEvent *event)
{
...
void
LayoutRegionBase::mouseMoveEvent (QGraphicsSceneMouseEvent *event)
{
  if (_selectedRegion != NULL)
  {
...
void
LayoutRegionBase::contextMenuEvent (QGraphicsSceneContextMenuEvent *event)
{
...
void
LayoutRegionBase::requestMediaOverRegionAction (QString mediaId,
                                                LayoutRegion *region)
{
...
void
LayoutRegionBase::keyPressEvent (QKeyEvent *event)
{
  // CTRL+C - Copy
...
bool
LayoutRegionBase::isGridVisible ()
{
  return _grid->isVisible ();
}

void
LayoutRegionBase::setGridVisible (bool visible)
{
...
QRect
LayoutRegionBase::getSafeAreaRect (int w, int h, double w_perc, double h_perc)
{
  double safeArea_x1 = ((1 - w_perc) * (double)w) / 2;
  double safeArea_y1 = ((1 - h_perc) * (double)h) / 2;
  return QRect (safeArea_x1, safeArea_y1, w_perc * (double)w,
                h_perc * (double)h);
}

void
LayoutRegionBase::createSafeArea ()
{
  // Draw safe area
  _safeActionAreaRect = new QGraphicsRectItem (
      getSafeAreaRect (this->width (), this->height (), 0.93, 0.93));
  _safeActionAreaRect->setPen (
      QPen (QBrush (QColor (0, 0, 255)), 1.5, Qt::DashDotDotLine));
  _safeActionAreaRect->setBrush (QColor (203, 203, 255, 50));
  _safeActionAreaRect->setZValue (1000);
  _safeActionAreaRect->setVisible (false);

  _safeActionAreaText
      = new QGraphicsTextItem (tr ("Safe Action Area"), _safeActionAreaRect);
  ...
  _safeTitleAreaRect = new QGraphicsRectItem (
      getSafeAreaRect (this->width (), this->height (), 0.9, 0.9));
  _safeTitleAreaRect->setPen (
      QPen (QBrush (QColor (255, 0, 0)), 1.5, Qt::DashDotDotLine));
  _safeTitleAreaRect->setBrush (QColor (255, 203, 203, 50));
  _safeTitleAreaRect->setZValue (1000);
  _safeTitleAreaRect->setVisible (false);

  _safeTitleAreaText
      = new QGraphicsTextItem (tr ("Safe Title Area"), _safeTitleAreaRect);
...
void
LayoutRegionBase::updateSafeAreaPos ()
{
  QRect newSafeActionRect
      = getSafeAreaRect (this->width (), this->height (), 0.93, 0.93);
  ...
  QRect newSafeTitleRect
      = getSafeAreaRect (this->width (), this->height (), 0.90, 0.90);
...
void
LayoutRegionBase::performSafeArea ()
{
...
bool
LayoutRegionBase::isSafeAreaVisible ()
{
  return _safeActionAreaRect->isVisible ();
}

void
LayoutRegionBase::setSafeAreaVisible (bool active)
{

...
LayoutWindow::LayoutWindow (QWidget *parent)
    : QMainWindow (parent), ui (new Ui::QnlyMainWindow)
{
  ui->setupUi (this);
...
LayoutWindow::~LayoutWindow () { delete ui; }

void
LayoutWindow::setQnlyView (LayoutView *view)
{
  assert (view != NULL);
  ...
  connect (ui->actionAdd_Region, SIGNAL (triggered ()), this,
           SLOT (addRegion ()));

  connect (ui->actionRemove_Region, SIGNAL (triggered ()), this,
           SLOT (removeSelectedRegion ()));

  connect (ui->actionShow_Hide_Grid, SIGNAL (toggled (bool)), view,
           SLOT (setGridVisible (bool)));

  connect (view, SIGNAL (gridVisibilityChanged (bool)),
           ui->actionShow_Hide_Grid, SLOT (setChecked (bool)));

  connect (ui->regionBaseComboBox, SIGNAL (activated (int)), this,
           SLOT (comboBoxChangedCurrentRegionBase (int)));
}

void
LayoutWindow::loadPreferences ()
{
...
void
LayoutWindow::removeSelectedRegion ()
{
  LayoutRegion *region = view->getSelectedRegion ();
  if (region != NULL)
  {
...
void
LayoutWindow::addRegionBaseToCombobox (
    const QString &uuid, const QMap<QString, QString> &properties)
{
  int comboCount = ui->regionBaseComboBox->count ();

  if (properties.count ("id"))
    ui->regionBaseComboBox->insertItem (
        comboCount - 2, QIcon (":/icon/regionbase"), properties["id"], uuid);
  else
    ui->regionBaseComboBox->insertItem (
        comboCount - 2, QIcon (":/icon/regionbase"), tr ("Unknown"), uuid);
}

void
LayoutWindow::removeRegionBaseFromCombobox (const QString &uuid)
{
...
void
LayoutWindow::comboBoxChangedCurrentRegionBase (int pos)
{
...
void
LayoutWindow::selectRegionBaseInComboBox (const QString &uuid)
{
...
void
LayoutWindow::updateRegionBaseInComboBox (
    const QString &uuid, const QMap<QString, QString> &properties)
{
  int index = ui->regionBaseComboBox->findData (uuid);
  if (index != -1)
...
void
LayoutWindow::on_actionAction_Export_triggered ()
{
  view->doSnapshot ();
}

void
LayoutWindow::on_actionCopy_region_triggered ()
{
  view->performCopy ();
}

void
LayoutWindow::on_actionPaste_region_triggered ()
{
  view->performPaste ();
}

void
LayoutWindow::on_resolutionComboBox_activated (const QString &arg1)
{
...
void
LayoutWindow::on_actionZoom_in_triggered ()
{
  if (view->getSelectedRegionBase () != NULL)
  {
    LayoutCanvas *zoomObject
        = (LayoutCanvas *)view->getSelectedRegionBase ()->parent ();
    zoomObject->performZoomIn ();
  }
}

void
LayoutWindow::on_actionZoom_out_triggered ()
{
  if (view->getSelectedRegionBase () != NULL)
  {
    LayoutCanvas *zoomObject
...
void
LayoutWindow::on_actionActionZoom_original_triggered ()
{
  if (view->getSelectedRegionBase () != NULL)
  {
    LayoutCanvas *zoomObject
...
void
LayoutWindow::on_actionCut_region_triggered ()
{
  view->performCut ();
}

void
LayoutWindow::on_actionSafe_area_triggered ()
{

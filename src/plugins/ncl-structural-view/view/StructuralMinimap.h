...
/** @brief Minimap for the current viewed structural scene
  *
  * Displays the scene of current diagram. The area of the scene, visible in
 * the
  * EditorView, displayed on the minimap as a rectangle. Navigation on the
 * scene is
  * possible
...
protected:
  ...
  /// painting out the areas which aren't to be painted on the minimap (not in
  /// the scene rect)
  void drawNonExistentAreas (QPainter *painter, QRectF const &rect);
  /// @return list of areas visible on the minimap but not included in the
  /// scene rectangle

...
void StructuralViewPlugin::updateFromModel()
{
  ...
  foreach (QString key, entities.keys())
  {
    StructuralGraphicsEntity *entity =
        dynamic_cast <StructuralGraphicsEntity*>(view->getEntity(entities.value(key)));
...
{    
  ...
  if(entities.contains(entity->getUniqueId()))
  {
    ...
    if(view->hasEntity(structuralId))
    {
      StructuralEntity *nstEntity = view->getEntity(structuralId);
...
void StructuralViewPlugin::requestEntityAddition(Entity* entity)
{
  ...
  Structural::EntityType type = StructuralUtil::getnstTypeFromStr(entity->getType());
  properties["TYPE"] = StructuralUtil::getStrFromNstType(type);
                                              ...
                                              QMap<QString, QString> properties)
{
  ...
  StructuralEntity *qnstEntity = view->entities.value(uid);
  if(qnstEntity != NULL)
  {
    //Disabled for now

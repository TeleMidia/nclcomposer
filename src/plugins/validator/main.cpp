...
void
validate (QString document, QString language)
{
  ...
  cout << "--"
       << " "
       << "nclvs - NCL Validation Service" << endl;
  cout << "--"
       << " "
       << "Version: 1.0.0" << endl;
  cout << "--" << endl;
  ...
  cout << "--"
       << " "
       << "Date: " << QDateTime::currentDateTime ().toString () << endl;
  cout << "--" << endl;
  cout << endl;
  ...
  if (file->open (QIODevice::ReadOnly))
  {
    ...
    while (!s.isEmpty ())
    {
      ...
      QDomElement e = qp.second;
      string name = e.nodeName ().toStdString ();
      ...
      m.element (uid)->setData (
          (void *)new pair<int, int> (e.lineNumber (), e.columnNumber ()));
      ...
      for (int i = list.length () - 1; i >= 0; --i)
      {
        if (list.item (i).isElement ())
        {
          ...
          s.push (
              QPair<QString, QDomElement> (QString (uid.c_str ()), element));
        }
      }
    }
...
main (int argc, char *argv[])
{
  ...
  if (argc > 1)
  {
    ...
    if (QString (argv[1]) == "-l")
    {
      if (QString (argv[2]) != "en" || QString (argv[2]) != "es"

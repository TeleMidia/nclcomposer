...
void
anyScopeReferenceValidation (const ModelElement &el,
                             const ModelElement &pointed, const Attribute &att,
                             Model &model, vector<pair<void *, string> > &msg,
                             Message &messageFactory)
{
  ...
  if (att.name () == "refer")
  {
    ...
    if (pointed.attribute ("refer").name () != "")
    {
      ...
      msg.push_back (make_pair (
          el.data (),
          messageFactory.createMessage (4109, 1, att.value ().c_str ())));
    ...
    while (element)
    {
      if (element->id () == pointed.id ())
      {
        ...
        msg.push_back (make_pair (el.data (),
                                  messageFactory.createMessage (3006, 0, "")));
        model.addElementWithErrorInLastPass (el.id ());
 ...
 */
void
specificScopeReferenceValidation (const ModelElement &el,
                                  const ModelElement &pointed,
                                  const Attribute &att,
                                  const ReferenceStructure &ref, Model &model,
                                  ...
                                  Message messageFactory)
{
  ...
  else if (ref.getPerspective () == "$GRANDPARENT")
  {
    ...
    scopeElement = scopeElement ? model.element (scopeElement->parent ())
                                : scopeElement;
  }
  ...
  string scopeAtt
      = scopeElement->attribute (ref.getPerspectiveAtt ()).value ();
  ...
  vector<ModelElement *> aux = model.elementsByIdentifier (scopeAtt);
  if (aux.empty ())
  {
    fprintf (stderr, "Unknown scope of '%s' attribute of '%s' element\n",
             att.name ().c_str (), el.elementName ().c_str ());
  ...
  if (pointed.scope () != scopeElement->id ())
  {
    ...
    if (attr.value () != "")
    {
      vector<ModelElement *> refs = model.elementsByIdentifier (attr.value ());
      if (!refs.empty ())
      {
    ...
    fprintf (stderr, "'%s' attribute of '%s' element should point to '%s' "
                     "'%s'\n",
             att.name ().c_str (), el.elementName ().c_str (),
             scopeAtt.c_str (), scopeElement->elementName ().c_str ());

    msgs.push_back (pair<void *, string> (
        el.data (),
 ...
 */
void
referenceValidation (const ModelElement &el, const Attribute &att,
                     Model &model, vector<pair<void *, string> > &msgs,
                     Message &messageFactory)
{
  vector<ModelElement *> pointedElements
      = model.elementsByIdentifier (att.value ());

  if (att.name () == "interface" && pointedElements.size () == 0)
  {
    pointedElements = model.elementsByPropertyName (
        el.attribute ("component").value (), att.value ());
  }
  ...
  if (pointedElements.empty ())
  {
    vector<ReferenceStructure> references
        = Langstruct::getReferences (el.elementName (), att.name ());
  ...
  const ReferenceStructure *ref = Langstruct::getReference (
      el.elementName (), att.name (), pointed->elementName ());

  /* Test if 'pointed' element can be pointed by 'el'.
   * If 'ref' is NULL, then don't exist a reference 'el.att.name' -> pointed.*/
  if (!ref)
  {
    fprintf (stderr, "'%s' attribute of '%s' element can't point to '%s' "
                     "element\n",
             att.name ().c_str (), el.elementName ().c_str (),
  ...
  ////
  //	* $PARENT.att: The scope element is defined by the 'att' of
  ////
  //                                                       'el.parent'
  //                                                       //
  //	* $GRANDPARENT.att: The scope element is defined by the 'att'
  ////
  //                                                   'el.parent.parent'
  //                                                   //
  ////////////////////////////////////////////////////////////////////////////
   ...
   * any position on the doc, so perspectives won't be a problem .*/
  if (ref->getPerspective () == "ANY")
    anyScopeReferenceValidation (el, *pointed, att, model, msgs,
                                 messageFactory);
  ...
  else if (ref->getPerspective () == "SAME")
  {
    ...
    {
      fprintf (stderr,
               "'%s' element pointed by '%s' attribute of '%s' element "
               "MUST be at same perspective of him\n",
               pointed->elementName ().c_str (), att.name ().c_str (),
               el.elementName ().c_str ());
...
void
testRepeatedRoles (const ModelElement &element, const ModelElement &connector,
                   Model &model, set<string> &roles,
                   vector<pair<void *, string> > &msgs,
                   Message &messageFactory)
{
  ...
  for (size_t i = 0; i < children.size (); i++)
  {
    ...
    if (child)
    {
      ...
      testRepeatedRoles (*child, connector, model, roles, msgs,
                         messageFactory);
    }
  }
}
...
void
linkValidation (const ModelElement &link, string connectorId, Model &model,
                vector<pair<void *, string> > &msgs, Message &messageFactory)
{
  ...
  map<string, pair<int, int> >::iterator it = roles.begin ();
  for (; it != roles.end (); it++)
  {
    ...
    if (min > count)
    {
      msgs.push_back (pair<void *, string> (
          ...
          messageFactory.createMessage (
              3902, 3, QString::number (count).toStdString ().c_str (),
              it->first.c_str (),
    ...
    if (max < count)
    {
      msgs.push_back (pair<void *, string> (
          link.data (),
          messageFactory.createMessage (
              3903, 3, QString::number (count).toStdString ().c_str (),
              it->first.c_str (),
...
void
SemanticValidation::semanticValidation (const ModelElement &el, Model &model,
                                        vector<pair<void *, string> > &msgs,
                                        Message &messageFactory)
{
  ...
  else if (el.elementName () == "bind")
  {
    ...
    if (link)
    {
      ...
      if (xconnector.value () != "")
      {
        linkValidation (*link, xconnector.value (), model, msgs,
                        messageFactory);
      }
    }
  }

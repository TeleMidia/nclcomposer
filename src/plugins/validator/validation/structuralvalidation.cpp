 ...
 ******************************************************************************/
void
testAttributes (const ModelElement &el, Model &model,
                vector<pair<void *, string> > &msgs, Message &messageFactory)
{
       ...
       declaredAttsSet.insert (att.name ()), ++attsIt)
  {
    ...
    if (atts.count (att.name ()) == 0)
    {
      ...
      msgs.push_back (pair<void *, string> (
          el.data (),
          messageFactory.createMessage (2002, 2, att.name ().c_str (),
                                       ...
                                       el.elementName ()))
    {
      ...
      msgs.push_back (pair<void *, string> (
          el.data (),
          messageFactory.createMessage (2004, 2, att.name ().c_str (),
                                        el.elementName ().c_str ())));
      model.addElementWithErrorInLastPass (el.id ());
    }
    ...
    if (declaredAttsSet.count (att.name ()) != 0)
    {
      ...
      msgs.push_back (pair<void *, string> (
          el.data (),
          messageFactory.createMessage (2013, 2, att.name ().c_str (),
                                        el.elementName ().c_str ())));
      model.addElementWithErrorInLastPass (el.id ());
    }
  }
  ...
  for (; it != atts.end (); ++it)
  {
    ...
    if ((*it).second && declaredAttsSet.count (att) == 0)
    {
      ...
      msgs.push_back (pair<void *, string> (
          el.data (), messageFactory.createMessage (
                          2003, 2, att.c_str (), el.elementName ().c_str ())));
      model.addElementWithErrorInLastPass (el.id ());
    }
  }
...
void
testCardinality (const ModelElement &el, Model &model,
                 map<string, int> &cardinalityMap,
                 vector<pair<void *, string> > &msgs, Message &messageFactory)
{
  vector<string> childrenNames
      = Langstruct::getChildrenNames (el.elementName ());

  // Flags to know if 'el' has some child with these cardinalities.
  bool hasHashtagChild = false, hasAChild = false, hasBChild = false;

  // Counter to know the number of 'el' children that have these cardinalities.
  int countHashtagOperator = 0, countAOperator = 0, countBOperator = 0;

  // The names of 'el' children that have these cardinalities.
  string hashTagChildren = "", aChildren = "", bChildren = "";
...
/*******************************************************************************
 ** This function will performs a recursively syntactic validation on
 *elements.*
...
StructuralValidation::structuralValidation (
    const ModelElement &el, Model &model, vector<pair<void *, string> > &msgs,
    Message &messageFactory)
{
  ...
  if (!Langstruct::existElement (el.elementName ()))
  {
    ...
    msgs.push_back (pair<void *, string> (
        el.data (),
        messageFactory.createMessage (2001, 1, el.elementName ().c_str ())));
    model.addElementWithErrorInLastPass (el.id ());
  ...
  string parent
      = el.parent () != "" ? model.element (el.parent ())->elementName () : "";
  if (!Langstruct::existParent (el.elementName (), parent))
  {
    ...
    msgs.push_back (pair<void *, string> (
        el.data (),
        messageFactory.createMessage (2005, 2, el.elementName ().c_str (),
                                      parent.c_str ())));
    model.addElementWithErrorInLastPass (el.id ());
  }
  ...
  if (parentElement)
  {
    ...
    testCardinality (*parentElement, model, cardinalityMap, msgs,
                     messageFactory);
  }

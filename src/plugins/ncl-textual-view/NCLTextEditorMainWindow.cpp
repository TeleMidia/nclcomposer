...
//  Set window to fixed size
  ...
  this->setWindowFlags (
      Qt::CustomizeWindowHint); // Set window with no title bar
#endif
  ...
  connect (_textEdit, SIGNAL (textChanged ()), this,
...
void
NCLTextEditorMainWindow::closeEvent (QCloseEvent *event)
{
  if (maybeSave ())
  {
    writeSettings ();
...
void
NCLTextEditorMainWindow::newFile ()
{
  if (maybeSave ())
  {
    _textEdit->clear ();
...
bool
NCLTextEditorMainWindow::saveAs ()
{
...
void
NCLTextEditorMainWindow::documentWasModified ()
{
  setWindowModified (_textEdit->isModified ());
}

void
NCLTextEditorMainWindow::createActions ()
{
#ifdef NCLEDITOR_STANDALONE
  ...
  connect (_textEdit, SIGNAL (copyAvailable (bool)), _cutAct,
           SLOT (setEnabled (bool)));
  connect (_textEdit, SIGNAL (copyAvailable (bool)), _copyAct,
           SLOT (setEnabled (bool)));
  ...
  connect (_fullscreenAct, SIGNAL (triggered ()), this,
           SLOT (showInFullScreen ()));

  _editPreferencesAct = new QAction (tr ("&Preferences"), this);
  _editPreferencesAct->setStatusTip (tr ("Edit Preferences related to "
                                         "editors."));
  connect (_editPreferencesAct, SIGNAL (triggered ()), this,
...
#ifdef NCLEDITOR_STANDALONE
void
NCLTextEditorMainWindow::createMenus ()
{
...
void
NCLTextEditorMainWindow::createToolBars ()
{
#ifdef NCLEDITOR_STANDALONE
...
void
NCLTextEditorMainWindow::createStatusBar ()
{
  statusBar ()->showMessage (tr ("Ready"));
}

void
NCLTextEditorMainWindow::createOutlineView ()
{
#ifdef NCLEDITOR_STANDALONE
  ...
  insertNodeChildAct
      = new QAction (QIcon (":/images/save.png"), tr ("&Add child"), this);
  connect (insertNodeChildAct, SIGNAL (triggered ()), this,
           SLOT (insertElement ()));
...
void
NCLTextEditorMainWindow::createProblemsView ()
{
#ifdef NCLEDITOR_STANDALONE
...
void
NCLTextEditorMainWindow::createSearchBox ()
{
  ...
  _dockSearchBox->setSizePolicy (QSizePolicy::MinimumExpanding,
                                 QSizePolicy::Minimum);

  connect (_searchBox, SIGNAL (findNext (QString)), SLOT (findNext (QString)));

  connect (_searchBox, SIGNAL (findPrevious (QString)),
           SLOT (findPrevious (QString)));

  connect (_searchBox, SIGNAL (hideButtonClicked ()), SLOT (hideSearchBox ()));

  connect (_searchBox, SIGNAL (replace (QString, QString, bool)),
           SLOT (replace (QString, QString, bool)));

  connect (_searchBox, SIGNAL (replaceAll (QString, QString)),
...
void
NCLTextEditorMainWindow::readSettings ()
{
...
void
NCLTextEditorMainWindow::writeSettings ()
{
  QSettings settings (QSettings::IniFormat, QSettings::UserScope, "TeleMidia",
                      "NCL Editor");
...
bool
NCLTextEditorMainWindow::maybeSave ()
{
...
void
NCLTextEditorMainWindow::loadFile (const QString &fileName)
{
  QFile file (fileName);
...
bool
NCLTextEditorMainWindow::saveFile (const QString &fileName)
{
  QFile file (fileName);
...
void
NCLTextEditorMainWindow::setCurrentFile (const QString &fileName)
{
  _curFile = fileName;
...
QString
NCLTextEditorMainWindow::strippedName (const QString &fullFileName)
{
  return QFileInfo (fullFileName).fileName ();
}

void
NCLTextEditorMainWindow::showInFullScreen ()
{
  if (!isFullScreen ())
    showFullScreen ();
  else
...
#ifdef NCLEDITOR_STANDALONE
void
NCLTextEditorMainWindow::gotoLineOf (QTreeWidgetItem *item, int column)
{
  (void)column;
...
void
NCLTextEditorMainWindow::insertElement ()
{
#ifdef NCLEDITOR_STANDALONE
  bool ok;
  ...
  if (ok && !element.isEmpty ())
  {
    ...
    int endLine = textEdit->SendScintilla (
        QsciScintilla::SCI_GETLINEENDPOSITION, line - 1);
    int beginLine = textEdit->SendScintilla (
        QsciScintilla::SCI_POSITIONFROMLINE, line - 1);
    ...
    char ch = textEdit->SendScintilla (QsciScintilla::SCI_GETCHARAT,
                                       beginLine + end_element_column - 1);
    if (ch == '/')
    {
      QString endtag = ">";
      endtag += "<";
    ...
    // qCDebug (CPR_PLUGIN_TEXTUAL) << line << " " << beginLine << " " <<
    // endLine << " "
    //          << end_element_column << " ";
    ...
    int lineident = textEdit->SendScintilla (
        QsciScintilla::SCI_GETLINEINDENTATION, line - 1);
    textEdit->SendScintilla (QsciScintilla::SCI_GETLINEINDENTATION, line - 1);

    if (fix_next_line_indentation)
      textEdit->SendScintilla (QsciScintilla::SCI_SETLINEINDENTATION, line + 1,
                               lineident);

    textEdit->SendScintilla (QsciScintilla::SCI_SETLINEINDENTATION, line,
                             lineident + 8);
...
void
NCLTextEditorMainWindow::createTextView ()
{
...
#ifndef NCLEDITOR_STANDALONE
  QWidget *titleWidget
      = new QWidget (this); /*where this a QMainWindow object*/
...
void
NCLTextEditorMainWindow::showSearchBox ()
{
  _dockSearchBox->show ();
...
void
NCLTextEditorMainWindow::hideSearchBox ()
{
  _dockSearchBox->hide ();
  _textEdit->setFocus ();
}
...
bool
NCLTextEditorMainWindow::findNext (const QString &text)
{
  return _textEdit->findFirst (text, true, true, true, true);
}

void
NCLTextEditorMainWindow::findPrevious (const QString &text)
{
  int line, index;
...
void
NCLTextEditorMainWindow::replace (const QString &textSearch,
                                  const QString &textReplace, bool findNext)
{
...
void
NCLTextEditorMainWindow::replaceAll (const QString &textSearch,

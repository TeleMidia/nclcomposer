...
~~~~~~~~~~~~~~~~~~~~~~~~~
Para instalar o NCL Composer, vá até a página:
http://composer.telemidia.puc-rio.br/en/download e baixe a versão mais nova
para o seu sistema (atualmente são suportados Window, Linux (Debian e Ubuntu)
e Mac OS X.

Windows
^^^^^^^
Para Windows são disponíveis duas versões:

  * Um arquivo compactado (.ZIP), e um
	* instalador (.EXE).

Baixe a versão que melhor lhe convier. Recomendamos a versão .ZIP se você
deseja apenas testar o NCL Composer, ou não tem permissão de administrador no
sistema onde deseja instalar. Se você tem permissão de administrador,
recomendamos o instalador .EXE pois nele você terá a opcão de já criar atalhos
na Área de Trabalho e no Menu Iniciar.

Para instalar a partir do .ZIP é suficiente extraí-lo em qualquer pasta do seu
sistema onde queira instalar o NCL Composer.

Para instalar a partir do .EXE, execute esse arquivo, que você será guiado por
um passo-a-passo de instalacão.
...
^^^^^^^^^^^^^^^^^^^^^^^^^
No decorrer deste tutorial iremos criar uma única aplicação que evoluirá em
cada um dos nossos exemplos. Para tirar total proveito deste tutorial, você
precisará dos objetos de mídia que compõe essa aplicação. Um pacote apenas com
os objetos de mídia que farão parte da aplicação pode ser encontrado em:

  * link:exemplos/garrincha_nclcomposer_media_only.zip[garrincha_nclcomposer_media_only.zip] 

Enquanto um pacote completo, com todos os objetos de mídia e os projetos no
NCL Composer de cada um dos exemplos está disponível em:

  * link:exemplos/garrincha_nclcomposer_full.zip[garrincha_nclcomposer_full.zip] 
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Para finalizar o nosso exemplo inicial _Alô Garrincha_ só falta definirmos
onde esse vídeo da animação será exibido. A forma mais simples de
...
apresentação inicial do vídeo e começar a animação propriamente dita (i.e. 5s
após o início do vídeo da animação);
...
Para desenvolver esse exemplo, vamos partir do exemplo anterior, no qual já
temos o nosso objeto de mídia da animaćão. Sendo assim, temos que inserir os
outros objetos de mídia que também farão parte deste exemplo:

	* media/chorinho.mp3 (A música de fundo)
	* media/shoes.mp3
